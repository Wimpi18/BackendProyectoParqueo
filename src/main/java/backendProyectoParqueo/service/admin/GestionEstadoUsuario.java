...
public class GestionEstadoUsuario {
  ...
  @Transactional
  public Object cambiarEstadoGeneralUsuario(UUID usuarioIdAfectado, boolean activar, String motivo,
      UUID adminQueEjecutaId) {
    Administrador adminEjecutor = administradorRepository.findById(adminQueEjecutaId)
        .orElseThrow(
            () -> new RuntimeException("Administrador ejecutor con ID " + adminQueEjecutaId + " no encontrado."));
    ...
    String motivoParaBitacora = motivo;
    if (activar && (motivo == null || motivo.trim().isEmpty())) {
      motivoParaBitacora = "";
    }
    ...
    if (optCliente.isPresent()) {
      ...
      if (optParqueo.isPresent()) {
        ...
        TipoAccion accionBitacora = determinarTipoAccionParaCambioEstadoParqueo(nuevoEstadoParqueo,
            estadoAnteriorParqueo);
        registrarBitacora(usuarioBaseAfectado, adminEjecutor, accionBitacora, motivoParaBitacora);
        return parqueo;
      }
      throw new RuntimeException(
          "Cliente con ID " + usuarioIdAfectado + " no tiene un parqueo asociado para modificar.");
    }
    ...
    if (optAdminAfectado.isPresent()) {
      ...
      if (adminAfectado.isEsActivo() == activar) {
        throw new RuntimeException("La cuenta del administrador ya se encuentra en el estado solicitado ("
            + (activar ? "Activa" : "Inactiva") + ").");
      }
    ...
    if (optCajeroAfectado.isPresent()) {
      ...
      if (cajeroAfectado.isEsActivo() == activar) {
        throw new RuntimeException("La cuenta del cajero ya se encuentra en el estado solicitado ("
            + (activar ? "Activa" : "Inactiva") + ").");
      }
    ...
    throw new RuntimeException(
        "No se encontró un perfil de Cliente con Parqueo, Administrador o Cajero para el Usuario ID "
            + usuarioIdAfectado + " para modificar su estado.");
  }

  private void registrarBitacora(Usuario usuarioAfectado, Administrador adminEjecutor, TipoAccion tipoAccion,
      String motivo) {
  ...
  private TipoAccion determinarTipoAccionParaCambioEstadoParqueo(EstadoParqueo nuevoEstado,
      EstadoParqueo estadoAnterior) {
    switch (nuevoEstado) {
      case Bloqueado:
  ...
  @Transactional
  public Parqueo cambiarEstadoEspecificoParqueo(UUID usuarioIdCliente, EstadoParqueo estadoDeseadoParqueo,
      String motivo, UUID adminQueEjecutaId) {
    ...
    Parqueo parqueo = parqueoRepository.findByCliente_Id(clienteAfectado.getId())
        .orElseThrow(() -> new RuntimeException(
            "No se encontró un Parqueo asociado al Cliente con Usuario ID " + usuarioIdCliente));
    ...
    if (adminEjecutor.getUsuario().getId().equals(usuarioDelClienteAfectado.getId())) {
      throw new RuntimeException(
          "Un administrador no puede modificar el estado de un parqueo que le pertenece como cliente.");
    }
