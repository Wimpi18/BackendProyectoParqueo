...
import backendProyectoParqueo.dto.VehiculoDTO; // Mantener si se usa para getVehiculosActivosParaReporte
...
public List<Object> getTodosVehiculosDTOPorCliente(UUID clienteId) {
    // Asumiendo que vehiculoRepository tiene el método que devuelve List<VehiculoDTO>
    ...
    public List<ReporteEstadoCuentaVehiculoDTO> getEstadosCuentaPorPlacaVehiculo(String placa) {
        ...
        if (parqueosDelVehiculo.isEmpty()) {
            // Puedes decidir si devolver una lista vacía o lanzar una excepción/mensaje específico
    ...
    private ReporteEstadoCuentaVehiculoDTO generarReporteParaParqueo(Parqueo parqueo) {
        ...
        // ... (asignaciones iniciales a reporte: placa, tipoCliente, tipoVehiculo, ultimaActualizacion) ...
        ...
        if (parqueo.getEstado() == Parqueo.EstadoParqueo.Inactivo) {
            if (ultimoMesPagadoRegistrado != null) {
                ...
                // Si está inactivo y NO hay pagos, el fin de cálculo es el mes de inicio del parqueo.
        ...
        // (podría pasar si se inactiva y el último pago fue antes del mes de inicio, aunque es ilógico)
        ...
        if (tarifaAplicable == null && parqueo.getEstado() != Parqueo.EstadoParqueo.Inactivo) {
            System.err.println("Advertencia: No se encontró tarifa vigente para cliente tipo " +
                    ...
                    parqueo.getVehiculo().getTipo() + ". Los montos pendientes pueden no ser precisos para el parqueo ID: " + parqueo.getId());
        }
        ...
        while (!mesIterador.isAfter(mesFinCalculo)) {
            if (!mesesPagadosRegistrados.contains(mesIterador)) {
                ...
                // y el parqueo no estaba ya inactivo antes de este mesIterador (manejado por mesFinCalculo)
